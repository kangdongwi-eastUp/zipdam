        ...
        <div class="background">
            ...
            <div class="mobile-box">
                ...
                <div class="menu">
                    ...
                    <div><label><input type="checkbox" name="selection" data-idx="appForUse"><img src="/img/main/icon07.png" class="icon">사용검사</label></div>
                    <div><label><input type="checkbox" name="selection" data-idx="installScreen"><img src="/img/main/icon08.png" class="icon">방충망시공</label></div>
                    <div><label><input type="checkbox" name="selection" data-idx="wasteCollection"><img src="/img/main/icon09.png" class="icon">폐기물수거</label></div>
                </div>
                    ...
                    document.addEventListener('DOMContentLoaded', () => {
                        ...
                        checkboxes.forEach(checkbox => {
                            checkbox.addEventListener('change', function () {
                                ...
                                const multiSelectable = ['agreement', 'curing', 'approval'];
                                const currentIdx = this.dataset.idx;

                                if (this.checked) {
                                    if (!multiSelectable.includes(currentIdx)) {
                                        // 단일 선택 항목을 선택한 경우, 다른 모든 체크박스 해제
                                        ...
                                        // 다중 선택 항목을 선택한 경우, 단일 선택 항목들만 해제
                                        checkboxes.forEach(cb => {
                                            if (cb.checked && !multiSelectable.includes(cb.dataset.idx)) {
                                                cb.checked = false;
                                                cb.closest('label').classList.remove('checked');
                                            }
                                        });
                                ...
                                // 단일 선택 항목이 선택되었고, 버튼이 표시되면 스크롤 이동
                                if (this.checked && !multiSelectable.includes(currentIdx) && selectedItems.length > 0) {
